          ...
          <div className="py-0.5">
            {suggestions.map(thread => (
              <button
                ...
                className="w-full text-left px-2.5 py-1.5 hover:bg-tuco-cyan/5 flex flex-col gap-0.5 border-b border-neutral-50 last:border-0 transition-colors"
              >
                <p className={`font-display font-bold ${compact ? 'text-xs' : 'text-[10px]'} text-[#4D4747] line-clamp-1`}>
                  {thread.title}
                </p>
                <div className="flex items-center gap-1.5">
                  <span className={`${compact ? 'text-[10px]' : 'text-[9px]'} text-neutral-400 font-medium`}>
                    {thread.replies.length} replies
                  </span>
                  <span className={`${compact ? 'text-[10px]' : 'text-[9px]'} text-neutral-300`}>•</span>
                  <span className={`${compact ? 'text-[10px]' : 'text-[9px]'} text-neutral-400 font-medium`}>
                    {thread.category}
                  </span>
                </div>
