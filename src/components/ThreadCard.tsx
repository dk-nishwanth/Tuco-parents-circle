...
export function ThreadCard({ thread, onOpen, onVote, votedState, users = {} }: ThreadCardProps) {
  ...
  return (
    ...
    >
      {/* Decorative vertical band for pinned/hot items */}
      {thread.isPinned && (
      ...
      <div className="votes flex flex-col items-center justify-start gap-1 p-1 bg-neutral-50 border border-neutral-200/60 rounded-2xl shrink-0 h-fit self-start min-w-[40px]">
        <button
          ...
          className={`vote-btn p-1.5 rounded-xl active:scale-95 transition-all text-neutral-500 cursor-pointer ${
            votedState === 'up' ? 'bg-[#EAF7F0] text-[#1D6F42]' : 'hover:bg-tuco-cyan/10 hover:text-tuco-cyan'
          }`}
          title="Upvote"
        >
        ...
        <button
          ...
          className={`vote-btn p-1.5 rounded-xl active:scale-95 transition-all text-neutral-500 cursor-pointer ${
            votedState === 'down' ? 'bg-[#FDF2F2] text-[#9B1C1C]' : 'hover:bg-neutral-200 hover:text-tuco-orange'
          }`}
          title="Downvote"
        >
      ...
      {/* Main Metadata & Body text of Thread */}
